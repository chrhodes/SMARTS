...
#End Region
    ...
    Protected Sub DataBindControl(ByVal Source As Object, ByVal sqlConn As PLDbConnection)
        ...
        Select Case Source.ID
            ...
            Case "grdArtifacts"
                ...
                sqlTable = sqlCmd.ExecuteDataTable
            Case "grdObjects"
                sqlCmd = New PLDbCommand("GetItem_Contained " & TABLE_SHAPE & "," & ddlList.SelectedItem.Value & "," & ABSTRACTION_SYSTEMOBJECT, sqlConn)
                sqlTable = sqlCmd.ExecuteDataTable
            Case "grdRelations"
                sqlCmd = New PLDbCommand("GetItem_Relation NULL," & TABLE_SHAPE & "," & ddlList.SelectedItem.Value & "," & ABSTRACTION_SYSTEM, sqlConn)
                sqlTable = sqlCmd.ExecuteDataTable
        End Select
        ...
        If E.CommandName = "Delete" Then
            ...
            Select Case Source.ID
                Case "grdArtifacts"
                    ...
                    Dim strSQL As String = "GetItem_Contained " & TABLE_SHAPE & "," & ddlList.SelectedItem.Value & "," & ABSTRACTION_SYSTEMOBJECT
            ...
            Select Case Source.ID
                Case "grdArtifacts"
                    ...
                    If intValue = EMPTY_INT Then
                        sqlConn.Close()
                        Response.Redirect("artifact.aspx?id=" & ddlList.SelectedItem.Value & "&type=" & SHAPETYPE_SYSTEM & "&artifact=" & EMPTY_INT, False)
                        Exit Sub
                    Else
                        ...
                        If booSetArtifact(sqlConn, ddlList.SelectedItem.Value, intValue) = True Then
                            ...
                            strSQL = "GetItem_Contained " & TABLE_SHAPE & "," & ddlList.SelectedItem.Value & "," & ABSTRACTION_SYSTEMOBJECT
                            sqlCmd = New PLDbCommand(strSQL, sqlConn)
                            Dim sqlTable As DataTable = sqlCmd.ExecuteDataTable
    ...
    Public Sub SubmitToDB(ByVal strName As String)
        ...
        With sqlCmd
            ...
            .AddParameter("@ShapeType", SqlDbType.Int, 0).Value = SHAPETYPE_SYSTEM
        ...
        Response.Redirect("sys.aspx?id=" & sqlCmd.Parameters("@NewID").Value.ToString() & "&type=" & CStr(SHAPETYPE_SYSTEM), False)
    ...
    Private Sub ddlList_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ddlList.SelectedIndexChanged
        ...
        Response.Redirect("sys.aspx?id=" & ddlList.SelectedItem.Value & "&type=" & CStr(SHAPETYPE_SYSTEM), False)
    ...
    Protected Sub DataBindPage(ByVal sqlConn As PLDbConnection)
        ...
        Else
            ...
            urlBrowseThis.NavigateUrl = "../SMARTSBROWSER/sys.aspx?id=" & Request.QueryString("id") & "&type=" & Request.QueryString("type")
            ...
            Case ListItemType.EditItem
                Select Case Source.id
                    Case "grdLifecycle"
                        ...
                        If grdRelations.Items.Count > 0 Or grdObjects.Items.Count Then
