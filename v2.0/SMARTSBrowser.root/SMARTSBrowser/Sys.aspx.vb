...
#End Region
    ...
    Protected Sub DataBindPage(ByVal sqlConn As PLDBConnection)
        ...
        strSQL = "EXEC GetItem_Contained " & TABLE_SHAPE & "," & Request.QueryString("id") & "," & ABSTRACTION_SYSTEMOBJECT
        sqlCmd = New PLDbCommand(strSQL, sqlConn)
        sqlTable = sqlCmd.ExecuteDataTable
        grdObjects.DataSource = sqlTable
        grdObjects.DataBind()

        strSQL = "EXEC GetItem_Relation NULL," & TABLE_SHAPE & "," & Request.QueryString("id") & "," & ABSTRACTION_SYSTEM
        sqlCmd = New PLDbCommand(strSQL, sqlConn)
        sqlTable = sqlCmd.ExecuteDataTable
